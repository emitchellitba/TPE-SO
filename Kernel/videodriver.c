...
void printStd(const char * buffer, size_t count) {
	print(buffer, count, 1);
}

void printErr(const char * buffer, size_t count) {
...
void scroll_down() {
	...
	for(int y = top_left_screen; first || y != top_left_screen; y++, y%=MAX_HEIGHT_LOG){
		if(first)
			first = 0;
	...
	posY = 0;
	first = 1;
	for(int y = top_left_screen; first || y != (top_left_screen ? top_left_screen - 1: MAX_HEIGHT_LOG - 1); y++, y%=MAX_HEIGHT_LOG){
		if(first)
			first = 0;
		for(int x = 0; x < MAX_WIDTH_LOG; x++) {
			if(screen_text_log[y][x] == '\n') {
...
int can_move_top_left_pointer() {									//esta funcion corrige posibles errores/bugs que tenga el borrado de caracteres con respecto a las posiciones de top_left_screen y currentY
	return currentY == MAX_HEIGHT_LOG || (mooving_matrix && (abs(currentY - top_left_screen) <= 1 || (currentY == MAX_HEIGHT_LOG - 1 && top_left_screen == 0)));
}
...
void update_screen_text_log(const char c) {
	...
	switch(c) {
		case '\n': {
			...
			currentY %= MAX_HEIGHT_LOG;
			top_left_screen %= MAX_HEIGHT_LOG;								//si se me pasó de la pantalla, lo vuelvo al principio
			break;	
		}
		case '\b': {
			if(currentX == 0) {
				if(currentY == 0 && mooving_matrix) {
					...
					currentX = MAX_WIDTH_LOG - 1;
					screen_text_log[currentY][currentX] = ' ';				//borramos el útlimo caracter guardado
				}
			} else {
				currentX--;
				screen_text_log[currentY][currentX] = ' ';
	...
	if(top_left_screen)														//una vez corres el principio de la pantalla, se va a seguir corriendo cada vez que aumente *y*
		mooving_matrix = 1;
}
        ...
        for (int i = 0; i < size; i++) { 
            ...
            for (cx = 0; cx < 8; cx++) { 
                for (int j = 0; j < size; j++) { 
                    putPixel(glyph[cy] & mask[cx] ? font_color : background_color, posX, posY);
                    posX++; 
                }
            }
            posY++; 
