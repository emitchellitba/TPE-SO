...
void press_key(){
	...
	if (scanCode <= 0x80){
		...
		}else if (!(scanCode == LEFT_SHIFT_PRESSED || scanCode == RIGHT_SHIFT_PRESSED || scanCode == LEFT_SHIFT_RELEASED || scanCode == RIGHT_SHIFT_RELEASED || scanCode == CAPSLOCK_PRESSED)){
			if (printableAscii[scanCode][0] >= 'a' && printableAscii[scanCode][0] <= 'z'){
