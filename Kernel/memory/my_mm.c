    ...
    if (found) {
      for (int j = 0; j < blocks_needed; j++) {
        ...
        if (j == 0) {
          ...
          kmm.block_info[i + j].block_count =
              0; // Concurrent blocks marked as used, but not the first
        }
      }
...
void my_kmm_free(memory_manager_adt mm, void *ptr) {
  if (!ptr || ptr < kmm.memory || ptr >= kmm.memory + HEAP_SIZE)
    return;
...
void my_kmm_dump_state(memory_manager_adt mem) {
  ...
  for (int i = 0; i < TOTAL_BLOCKS; i++) {
    print(kmm.block_info[i].is_free ? (const char *)'.' : (const char *)'#', 1,
          0);
  }
  print_str("\n", 1, 0);
}
