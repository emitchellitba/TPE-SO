...
static uint64_t tick_frequency =
    18; // Frecuencia del PIT en Hz (18.2 Hz por defecto)
...
uint64_t seconds_to_ticks(uint64_t seconds) { return seconds * tick_frequency; }

uint64_t ticks_to_seconds(uint64_t ticks) { return ticks / tick_frequency; }
