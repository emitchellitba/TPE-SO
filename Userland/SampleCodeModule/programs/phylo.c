...
void philosopher_loop(int id, char *pipe_id, int n_forks) {
  ...
  int left_mutex = my_sem_open(left_name);
  int right_mutex = my_sem_open(right_name);
...
void add_philosopher() {
  ...
  fork_mutex[n_philosophers] =
      my_sem_create(fork_mutex_name[n_philosophers], 1);
...
int phylo_main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < n_philosophers; i++) {
    ...
    fork_mutex[i] = my_sem_create(fork_mutex_name[i], 1);
  }
